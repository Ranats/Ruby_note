...
#    @pareto = [@pareto,genes].max_by{|gene| gene.score[i]}
...
class ViewController
  def initialize
    @root = TkRoot.new(title:"Ruby/Tk KP100")
    @generation_label = TkLabel.new(@root).pack
    @canvas = TkCanvas.new(@root, width:300, height:300).pack
...
#  Parallel.each([*0..10], in_processes: 4) do |t|

  agent = DCMOGA.new(population: 100, to_migrate: 4000, time: 1)

#  Gnuplot.open do |gp|
#    Gnuplot::Plot.new(gp) do |plot|
#      plot.ylabel 'knapsack1'
#      plot.xlabel 'knapsack2'
#        plot.terminal ''
#      plot.xrange '[0:4500]'
#      plot.yrange '[0:4500]'
...
#        plot.title "generation: #{generation}"

        score = [agent.moga.pareto.collect{|gene| gene.score[0]}, agent.moga.pareto.collect{|gene| gene.score[1]}]
#        replot.data << Gnuplot::DataSet.new( [score[0], score[1]]) do |ds|
#          ds.with = "points pt 7"
#          ds.notitle
#        end
    File.open("output.dat","w") do |file|
      score[0].size.times do |i|
        file.puts "#{score[0][i]} #{score[1][i]}"
      end
    end
  end
#    end
